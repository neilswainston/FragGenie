...
public class CombinedSingleCandidateMetFragProcess implements Runnable {
	...
	/**
	 * each candidate processing thread has its own settings object and the
	 * candidate to process
	 * 
	 * @param settings
	 * @param candidate
	 */
	public CombinedSingleCandidateMetFragProcess(final Settings settings, final ICandidate candidate,
			final SortedTandemMassPeakList peakList, final Level logLevel) {
	...
	public void run() {
		try {
			final TopDownFragmenterAssignerScorer scorer = new TopDownFragmenterAssignerScorer(this.settings,
					this.candidate, this.peakList);
			scorer.calculate();
