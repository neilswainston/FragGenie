...
public class HDTopDownBitArrayFragmentWrapper extends AbstractTopDownBitArrayFragmentWrapper {
	...
	public HDTopDownBitArrayFragmentWrapper(AbstractTopDownBitArrayFragment wrappedFragment,
			int currentPeakIndexPointer, int[] currentPeakIndexPointerArrayHD) {
		super(wrappedFragment, currentPeakIndexPointer);
	...
	public byte matchToPeak(IMolecularStructure precursorMolecule, IPeak peak, int precursorIonTypeIndex,
		...
		if (numberDeuteriums == 0 && Constants.ADDUCT_NOMINAL_MASSES.get(precursorIonTypeIndex) == -2)
			ionisationTypeMassCorrection[0] = Constants
					.getIonisationTypeMassCorrection(Constants.ADDUCT_NOMINAL_MASSES.indexOf(-1), isPositive);
		 ...
		 */
		for (int i = 1; i <= this.wrappedFragment.getTreeDepth(); i++)
			if (i + (this.wrappedFragment.getNumberHydrogens() - variableDeuteriums) > currentPrecursor
					.getNumberHydrogens()) {
			...
			for (int i = 0; i < toUseForDeuterium.length; i++) {
				...
				for (int j = 0; j < toUseForHydrogen.length; j++) {
					...
					for (byte k = 0; k <= variableDeuteriums; k++) {
						...
						if (shifts[i] + shifts[j] + this.wrappedFragment.getNumberHydrogens()
								- substractHydrogenFromCharge < 0) {
							continue;
						}
						// check number normal hydrogens smaller zero (shifts[j] -> number shifted
						// hydrogens)
						if (shifts[j] + (this.wrappedFragment.getNumberHydrogens() - numberDeuteriums)
								- substractHydrogenFromCharge < 0) {
							continue;
						}
						// check number overall hydrogens greater than hydrogens available
						if ((int) (shifts[i] + shifts[j]
								+ this.wrappedFragment.getNumberHydrogens()) > numberHydrogensPrecusor) {
							continue;
						}
						...
						// number of overall deuteriums of fragment including shift, variable, real must
						...
						monoisotopicFragmentMass += Constants.getMonoisotopicMassOfAtom("D") * numberDeuteriums //$NON-NLS-1$
								- Constants.getMonoisotopicMassOfAtom("H") * numberDeuteriums; //$NON-NLS-1$

						double currentFragmentMass = monoisotopicFragmentMass + ionisationTypeMassCorrection[p]
								+ (shifts[j]) * Constants.getMonoisotopicMassOfAtom("H") //$NON-NLS-1$
								+ (Constants.getMonoisotopicMassOfAtom("D") * k //$NON-NLS-1$
										- Constants.getMonoisotopicMassOfAtom("H") * k) //$NON-NLS-1$
								+ (shifts[i] * Constants.getMonoisotopicMassOfAtom("D")); //$NON-NLS-1$
						byte compareResult = ((TandemMassPeak) peak).matchesToMass(currentFragmentMass);
						if (compareResult == 0) {
							if (fragmentPeakMatch[0] != null) {
								((HDFragmentMassToPeakMatch) fragmentPeakMatch[0]).addMatchedFragment(
										this.wrappedFragment, (byte) shifts[j], (byte) shifts[i], currentFragmentMass,
										p == 0 ? (byte) precursorIonTypeIndex : (byte) 0, k, numberDeuteriums);
							} else {
	...
	public HDFragmentMolecularFormulaWrapper getMolecularFormula(IMolecularStructure precursorMolecule)
			throws AtomTypeNotKnownFromInputListException {
		HDTopDownBitArrayPrecursor currentPrecursor = (HDTopDownBitArrayPrecursor) precursorMolecule;
		BitArrayFragmentMolecularFormula formula = new BitArrayFragmentMolecularFormula(currentPrecursor,
				this.wrappedFragment.getAtomsFastBitArray());
		byte numberDeuteriums = 0;
		for (int i = 0; i < this.wrappedFragment.getAtomsFastBitArray().getSize(); i++)
			if (this.wrappedFragment.getAtomsFastBitArray().get(i))
				numberDeuteriums += currentPrecursor.getNumberDeuteriumsConnectedToAtomIndex(this.precursorIndex, i);
