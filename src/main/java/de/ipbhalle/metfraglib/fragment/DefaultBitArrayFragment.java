...
public class DefaultBitArrayFragment extends AbstractFragment {
	...
	public DefaultBitArrayFragment(BitArrayPrecursor precursorMolecule,
			...
			if (this.atomsFastBitArray.get(i)) {
				for (int k = 0; k < atomIndeces.length; k++) {
					if (this.atomsFastBitArray.get(atomIndeces[k]))
						this.bondsFastBitArray.set(
								precursorMolecule.getBondIndexFromAtomAdjacencyList((short) i, atomIndeces[k]) - 1);
					else
						this.brokenBondsFastBitArray.set(
								precursorMolecule.getBondIndexFromAtomAdjacencyList((short) i, atomIndeces[k]) - 1);
				}
			} else {
				for (int k = 0; k < atomIndeces.length; k++) {
	...
	/*
	 * public void initialiseMolecularFormula(IMolecularStructure precursorMolecule)
	 * throws AtomTypeNotKnownFromInputListException { this.molecularFormula = new
	 * BitArrayFragmentMolecularFormula((BitArrayPrecursor)precursorMolecule,
	 * this.atomsFastBitArray); }
	 */

	@Override
	public byte matchToPeak(IMolecularStructure precursorMolecule, IPeak peak, int precursorIonTypeIndex,
			boolean isPositive, IMatch[] fragmentPeakMatch) {
		...
		for (int i = 0; i < ionisationTypeMassCorrection.length; i++) {
			...
			if (compareResult == 0 && substractHydrogenFromCharge <= numberHydrogens) {
				/*
				 * if a former fragment has matched already then add the current fragment list
				 * to the match object
				 */
				if (fragmentPeakMatch[0] != null) {
					((FragmentMassToPeakMatch) fragmentPeakMatch[0]).addMatchedFragment(this, (byte) 0,
							currentFragmentMass, i == 0 ? (byte) precursorIonTypeIndex : (byte) 0);
				}
				/*
				 * if no former fragment matched before initialise the match object and add the
				 * fragment list
				 */
				else {
			...
			for (byte hydrogenShift = 1; hydrogenShift <= maximalHydrogenShift; hydrogenShift++) {
				...
				for (byte signIndex = 0; signIndex < signs.length; signIndex++) {
					 ...
					 */
					double currentFragmentMassIonModeCorrected = currentFragmentMass
							+ signs[signIndex] * hydrogenShift * Constants.HYDROGEN_MASS;
					compareResult = ((TandemMassPeak) peak).matchesToMass(currentFragmentMassIonModeCorrected);
					numberComparisons++;
					/*
					 * in case fragment matches to peak create match or add the fragment to the
					 * match if result is 0 then the fragment mass matched to the peak
					 */
					if (compareResult == 0) {
						/*
						 * if a former fragment has matched already then add the current fragment list
						 ...
						 */
						if (fragmentPeakMatch[0] != null) {
							((FragmentMassToPeakMatch) fragmentPeakMatch[0]).addMatchedFragment(this,
									(byte) (signs[signIndex] * hydrogenShift), currentFragmentMassIonModeCorrected,
									i == 0 ? (byte) precursorIonTypeIndex : (byte) 0);
						}
						/*
						 * if no former fragment matched before initialise the match object and add the
						 * fragment list
					...
					/*
					 * if hydrogen removed/added from fragment mass and fragment mass
					 * smaller/greater than peak mass there is no need to check further by
					 * removing/adding hydrogens
					 */
	...
	public String getPreparedSmiles(IMolecularStructure precursorMolecule, IAtom atomToAdd, Integer atomPositionToAdd)
			throws CDKException {
	...
	public IAtomContainer getStructureAsIAtomContainer(IMolecularStructure precursorMolecule, IAtom atomToAdd,
		...
		java.util.List<IAtom> atomsToAdd = new java.util.LinkedList<>();
		java.util.List<IBond> bondsToAdd = new java.util.LinkedList<>();
		...
		for (IAtom atom : atomsToAdd)
			fragmentStructure.addAtom(atom);
		atomsToAdd = new java.util.LinkedList<>();
		bondsToAdd = new java.util.LinkedList<>();
		for (int i = 0; i < fragmentStructure.getAtomCount(); i++) {
	...
	public boolean equals(Object object) {
		if (this.atomsFastBitArray.equals(((DefaultBitArrayFragment) object).getAtomsFastBitArray()))
			return true;
		return false;
	}

	/**
	 * returns 0 if the mass of the current is equal to the mass of the argument
	 * fragment returns -1 if the mass of the current is smaller to the mass of the
	 * argument fragment returns 1 if the mass of the current is greater to the mass
	 * of the argument fragment
	 * 
	...
	public byte shareEqualProperties(IMolecularStructure precursorMolecule, IFragment fragment) {
		if (this.getMonoisotopicMass(precursorMolecule) == fragment.getMonoisotopicMass(precursorMolecule))
			return 0;
		if (this.getMonoisotopicMass(precursorMolecule) > fragment.getMonoisotopicMass(precursorMolecule))
			return 1;
		return -1;
	...
	public DefaultBitArrayFragment clone(IMolecularStructure precursorMolecule)
			throws AtomTypeNotKnownFromInputListException {
		DefaultBitArrayFragment clone = new DefaultBitArrayFragment(this.atomsFastBitArray.clone(),
				this.bondsFastBitArray.clone(), this.brokenBondsFastBitArray.clone());
		// clone.setID(this.ID);
	...
	public boolean isRealSubStructure(IFragment molecularStructure) {
		...
		if (currentAtomsFastBitArray.getSize() != this.atomsFastBitArray.getSize())
			return false;
		...
		for (int i = 0; i < this.atomsFastBitArray.getSize(); i++) {
			...
			if (thisAtomsFastBitArrayValue)
				thisAtomsFastBitArrayCardinality++;
			if (currentAtomsFastBitArrayValue)
				currentAtomsFastBitArrayCardinality++;
			if (!thisAtomsFastBitArrayValue && currentAtomsFastBitArrayValue)
				return false;
		}
		if (thisAtomsFastBitArrayCardinality == currentAtomsFastBitArrayCardinality)
			return false;
		return true;
	}
	...
	public int[] getUniqueBrokenBondIndeces(IFragment molecularStructure) {
		FastBitArray currentBrokenBondFastBitArray = ((DefaultBitArrayFragment) molecularStructure)
				.getBrokenBondsFastBitArray();
	...
	public IFragment getDifferenceFragment(IMolecularStructure precursorMolecule, IFragment molecularStructure) {
		FastBitArray diffFastBitArray = this.getAtomsFastBitArray()
				.getDiff(((DefaultBitArrayFragment) molecularStructure).getAtomsFastBitArray());
		DefaultBitArrayFragment diffFragment = new DefaultBitArrayFragment((BitArrayPrecursor) precursorMolecule,
				diffFastBitArray);
		if (!diffFragment.isConnected(precursorMolecule))
			return null;
		return diffFragment;
	}
	...
	public boolean isConnected(IMolecularStructure precursorMolecule) {
		...
		java.util.LinkedList<Integer> toCheck = new java.util.LinkedList<>();
