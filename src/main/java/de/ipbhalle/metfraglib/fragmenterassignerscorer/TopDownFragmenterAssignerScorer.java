...
public class TopDownFragmenterAssignerScorer {
	 ...
	 * 
	 */
	private final String scoreType = "FragmenterScore"; //$NON-NLS-1$
	...
	public TopDownFragmenterAssignerScorer(Settings settings, ICandidate candidate,
			final SortedTandemMassPeakList peakList) throws Exception {
	...
	public void calculate() throws AtomTypeNotKnownFromInputListException {
		AbstractTopDownBitArrayPrecursor candidatePrecursor = (AbstractTopDownBitArrayPrecursor) (this.candidate)
				.getPrecursorMolecule();
		// generate root fragment to start fragmentation
		AbstractTopDownBitArrayFragment root = candidatePrecursor.toFragment();
		Byte maximumTreeDepth = (Byte) this.settings.get(VariableNames.MAXIMUM_TREE_DEPTH_NAME);
		...
		// read peaklist
		this.peakList.initialiseMassLimits((Double) this.settings.get(VariableNames.RELATIVE_MASS_DEVIATION_NAME),
		...
		this.fragmenter.setMinimumFragmentMassLimit(
				this.fragmenter.getMinimumFragmentMassLimit() - Constants.ADDUCT_MASSES.get(precursorIonTypeIndex));
		...
		AbstractTopDownBitArrayFragmentWrapper rootFragmentWrapper = new AbstractTopDownBitArrayFragmentWrapper(root,
				this.peakList.getNumberElements() - 1);
			...
			while (!toProcessFragments.isEmpty()) {
				...
				for (int l = 0; l < fragmentsOfCurrentTreeDepth.size(); l++) {
					...
					while (matched != 1 && tempPeakPointer >= 0) {
						 ...
						 * calculate match
						 */
						matched = currentFragment.matchToPeak(candidatePrecursor,
								this.peakList.getElement(tempPeakPointer), precursorIonTypeIndex, positiveMode, match);
						/*
						 * check whether match has occurred
						 */
						if (matched == 0) {
							// currentFragment.setPrecursorFragments();
							...
							if (peakIndexToPeakMatch.containsKey(tempPeakPointer)) {
								Double[] values = peakIndexToPeakMatch.get(tempPeakPointer)
										.containsByFingerprint(currentFragment.getAtomsFastBitArray());
								if (values == null) {
									peakIndexToPeakMatch.get(tempPeakPointer).insert(newNode);
								} else {
									if (values[0] < currentScores[0][0]) {
										peakIndexToPeakMatch.get(tempPeakPointer)
												.removeElementByID((int) Math.floor(values[1]));
										fragmentIndexToPeakMatch.get((int) Math.floor(values[1]))
												.removeElementByID(tempPeakPointer);
										if (fragmentIndexToPeakMatch.get((int) Math.floor(values[1]))
												.getRootNode() == null) {
							...
							if (!similarFragmentFound) {
								if (fragmentIndexToPeakMatch.containsKey(currentFragment.getID())) {
									fragmentIndexToPeakMatch.get(currentFragment.getID()).insert(
											this.peakList.getElement(tempPeakPointer), currentScores[0][0],
											tempPeakPointer);
								} else {
									MatchPeakList newPeakList = new MatchPeakList(
											this.peakList.getElement(tempPeakPointer), currentScores[0][0],
											tempPeakPointer);
		...
		try {
			for (int i = 0; i < this.scoreCollection.getNumberScores(); i++) {
				...
				if (singleScores[i].length != 0 && this.scoreCollection.getScore(i).hasInterimResults()
						&& !this.scoreCollection.getScore(i).isInterimResultsCalculated()) {
					this.scoreCollection.getScore(i).setOptimalValues(singleScores[i]);
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
			this.logger.warn("candidate score calculation interrupted"); //$NON-NLS-1$
			return;
		}
	}
	...
	public void assignScores() {
		this.candidate.setProperty(this.scoreType, this.scoreCollection.getScore(0).getValue());
	}
	...
	public void assignInterimScoresResults() {
		this.candidate.setProperty(this.scoreType + "_Values", //$NON-NLS-1$
				this.scoreCollection.getScore(0).getOptimalValuesToString());
	}
}