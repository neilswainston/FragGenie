...
public class Fragmenter {
	 ...
	 * @throws Exception
	 */
	public Fragmenter(final PrecursorCandidate candidate, final int maximumTreeDepth) throws Exception {
		...
		this.detectedNeutralLosses = new NeutralLosses()
				.getMatchingAtoms(this.candidate.getPrecursorMolecule());
	}
	...
	public FragmentList generateFragments() throws Exception {
		 ...
		 */
		Fragment root = (Fragment) this.candidate.getPrecursorMolecule().toFragment();
		...
		for (int k = 1; k <= this.maximumTreeDepth; k++) {
			...
			while (!temporaryFragments.isEmpty()) {
				...
				for (int i = 0; i < indecesOfSetBondsOfNextTopDownFragment.length; i++) {
					 ...
					 */
					Fragment[] newGeneratedTopDownFragments = nextTopDownFragmentForFragmentation.traverseMolecule(
							this.candidate.getPrecursorMolecule(), nextBondIndexToRemove, indecesOfBondConnectedAtoms);
