	...
	private void initialise(Precursor precursorMolecule2)
		...
		for (int i = 0; i < precursorMolecule2.getStructureAsIAtomContainer().getAtomCount(); i++) {
			...
			if (byteToAtomSymbol == -1) {
				throw new de.ipbhalle.metfraglib.exceptions.AtomTypeNotKnownFromInputListException(
						currentAtomSymbol + " not found"); //$NON-NLS-1$
			}
			...
			this.numberHydrogens += precursorMolecule2.getStructureAsIAtomContainer().getAtom(i)
